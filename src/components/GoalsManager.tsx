...
import { Target, Plus, ChevronRight, Award, Trash2, Loader2, Pencil } from 'lucide-react';
    ...
    try {
      ...
      if (editingGoal) {
        ...
        const { data, error } = await supabase
          .from('goals')
          .insert({ ...payload, current: 0 })
          .select()
          .single();

        if (error) throw error;
        if (data) setGoals([...goals, data as WeeklyGoal]);
      }
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col items-center gap-4 w-full md:w-auto">
          <div className="flex bg-slate-800/50 p-1.5 rounded-2xl border border-slate-700/50 w-full md:w-auto">
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeGoalTab === 'weekly' ? 'bg-amber-400 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
            >
              Semanal
            </button>
            <button
              onClick={() => setActiveGoalTab('monthly')}
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeGoalTab === 'monthly' ? 'bg-amber-400 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
            >
              Mensal
            </button>
          </div>
          <button
            ...
            className="w-full md:w-auto px-8 py-4 bg-indigo-500 hover:bg-indigo-400 text-white font-black rounded-2xl transition-all flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20 active:scale-95"
          >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" onClick={() => { setShowAddModal(false); setEditingGoal(null); }}></div>
          <div className={`relative w-full max-w-sm border shadow-2xl p-8 animate-in zoom-in-95 duration-200 backdrop-blur-[40px] rounded-[2.5rem] ${isLight ? 'bg-white/90 border-white/40' : 'bg-slate-900/90 border-slate-700/50'}`}>
            <h3 className={`text-2xl font-black mb-6 ${isLight ? 'text-slate-900' : 'text-white'}`}>{editingGoal ? 'Editar Meta' : `Nova Meta ${activeGoalTab === 'weekly' ? 'Semanal' : 'Mensal'}`}</h3>
